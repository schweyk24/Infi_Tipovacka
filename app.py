...
# Funkce pro načtení dat s mezipamětí (šetříme Google limity)
def load_data():
    conn = st.connection("gsheets", type=GSheetsConnection)
    # TTL nastavíme na 60 sekund - aplikace nebude každou vteřinu otravovat Google
    ...
    # Sjednocení ID na text
    df_m['match_id'] = df_m['match_id'].astype(str)
    if not df_b.empty:
        df_b['match_id'] = df_b['match_id'].astype(str)
    return conn, df_m, df_b

try:
    conn, df_matches, df_bets = load_data()
