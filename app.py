...
st.markdown("""
    ...
    .admin-card { background-color: #fff9db; border: 1px solid #fcc419; padding: 15px; border-radius: 10px; margin-bottom: 15px; }
...
def load_data():
    ...
    # Users formátování (stringy bez .0)
    if not df_u.empty:
...
if not st.session_state.user and not st.session_state.admin:
            ...
            if st.form_submit_button("Vytvořit účet"):
                if u_r and p_r and len(ph_r) == 3:
                    if u_r.lower() in [n.lower() for n in df_u['user_name'].tolist()]:
                        st.warning("Tato přezdívka je již obsazená.")
                    else:
                        ...
                        st.cache_data.clear()
                        st.success("Registrace OK! Přejdi na záložku Přihlášení.")
                else: st.error("Všechna pole musí být vyplněna!")
    ...
    with t_adm:
        ...
        if st.button("Vstoupit do správy"):
            if a_pw == "hokej2026":
                st.session_state.admin = True; st.rerun()

# --- 2. ADMIN SEKCE ---
elif st.session_state.admin:
    st.header("⚙️ Správa zápasů")
    ...
    for _, m in to_score.iterrows():
        with st.container():
            ...
            if c3.button("Uložit & Spočítat", key=f"s{m['match_id']}"):
                ...
                st.cache_data.clear(); st.success("Hotovo!"); st.rerun()

# --- 3. HRÁČSKÁ SEKCE ---
else:
    u_row = df_u[df_u['user_name'] == st.session_state.user]
    pts = int(u_row['total_points'].values[0]) if not u_row.empty else 0
    st.markdown(f"<h3 style='text-align: center;'>Vítej, {st.session_state.user}! 🏒 | Máš {pts} bodů</h3>", unsafe_allow_html=True)
    ...
    with t1:
        ...
        for _, m in open_m.iterrows():
            ...
            if has_bet:
                st.markdown(f'<div class="bet-confirmed">✅ Tip: {int(user_bet.iloc[0]["tip_a"])} : {int(user_bet.iloc[0]["tip_b"])}</div>', unsafe_allow_html=True)
            else:
